...
create type public.activity_kind as enum ('routine', 'non_routine', 'emergency');
create type public.control_level as enum ('elimination', 'substitution', 'engineering', 'administrative', 'training', 'ppe');
create type public.risk_band as enum ('Bajo', 'Moderado', 'Importante', 'Intolerable');
...
create table public.sector_modules (
  ...
  hazard_groups text[] not null default '{}',
  suggested_evidence text[] not null default '{}',
  typical_controls text[] not null default '{}',
  validation_warning text not null default 'Modulo con referencias normativas pendientes de validacion articulo por articulo.',
  created_at timestamptz not null default now()
);
...
create table public.risk_assessments (
  ...
  exposure integer not null default 1 check (exposure between 1 and 5),
  initial_score integer generated always as (probability * severity * exposure) stored,
  initial_level risk_band not null,
  initial_acceptability text not null default 'No aceptable',
  ...
  residual_exposure integer check (residual_exposure between 1 and 5),
  residual_score integer generated always as (residual_probability * residual_severity * residual_exposure) stored,
  residual_level risk_band,
  residual_acceptability text,
  routine_type activity_kind,
  exposed_workers integer check (exposed_workers > 0),
  vulnerable_workers text,
  hazard_source text,
  hazardous_event text,
  existing_controls text,
  proposed_control_summary text,
  legal_validation_missing boolean not null default true,
...
create table public.action_plan_items (
  ...
  hierarchy_level control_level not null,
  responsible_person text not null,
  deadline date not null,
  status text not null default 'pending',
  verification_status text not null default 'Pendiente de evidencia',
  required_evidence text[] not null default '{}',
...
create table public.risk_control_links (
  ...
  control_order integer not null default 1,
  source_type text not null default 'rule_engine',
  created_at timestamptz not null default now(),
  unique (risk_assessment_id, control_id)
);

create table public.risk_legal_links (
  id uuid primary key default gen_random_uuid(),
  risk_assessment_id uuid not null references public.risk_assessments(id) on delete cascade,
  legal_norm_id uuid not null references public.legal_norms(id),
  legal_article_id uuid references public.legal_articles(id),
  obligation_snapshot text not null default 'requires legal validation',
  validation_status legal_source_status not null default 'requires_validation',
  created_at timestamptz not null default now()
);
...
create index workplaces_company_id_idx on public.workplaces(company_id);
...
create index sector_modules_code_idx on public.sector_modules(code);
create index evidence_requirements_applies_idx on public.evidence_requirements(applies_to);
...
create index action_plan_items_assessment_idx on public.action_plan_items(risk_assessment_id);
...
grant select on public.hazards, public.risks, public.controls, public.legal_norms, public.legal_articles, public.sector_modules, public.evidence_requirements to anon, authenticated;
